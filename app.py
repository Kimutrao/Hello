...
tab_basic, tab_mind, tab_realestate, tab_finance, tab_price = st.tabs([
    "📖 투자철학 요약", "🧠 기록 & 게시판", "🏢 부동산 & 대출", "💰 복리 계산기", "📊 시세비교"
])

# ═══════════════════════════════════════════
# 탭 1: 투자철학 요약 (선생님 블로그 내용 기반)
# ═══════════════════════════════════════════
with tab_basic:
...
with tab_mind:
    ...
    with col_menu:
        ...
        if selected_cat != st.session_state['selected_cat']:
            ...
            st.session_state['board_view'] = 'list' # 카테고리 변경 시 무조건 리스트로
            st.rerun()
    ...
    with col_board:
        ...
        if st.session_state['selected_cat'] == "전체글보기":
            display_posts = reversed(st.session_state['blog_db']) # 최신글이 위로
        else:
        ...
        # --- 3) 게시판 목록 화면 ---
            ...
            if not display_posts:
                ...
                for idx, post in enumerate(display_posts):
                    # HTML과 Streamlit 버튼을 섞어 게시판 행(Row) 생성
                    with st.container():
...
with tab_finance:
    ...
    with col_calc2:
        ...
        for m in range(1, total_months + 1):
            balance += monthly_addition # 1. 매월 적립금 추가
            invested_total += monthly_addition
            interest = balance * monthly_rate # 2. 추가된 금액에 이자 발생
            balance += interest
    ...
    line_total = base_chart.mark_line(color='#388E3C', strokeWidth=3).encode(
        y='총자산:Q'
    )
...
# ═══════════════════════════════════════════
with tab_price:
    st.info("이곳은 클로드(Claude)가 구축해 둔 국토부 데이터 필터링 및 산점도(Scatter) 그래프 영역입니다. 기능이 훌륭하여 그대로 유지했습니다.")
